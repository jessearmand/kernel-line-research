const POINTS = [
  ...
  },
  {
    ...
    body: "The kernel line is orthogonal to secrets and egress. The pattern that actually keeps a token out of the agent is the same everywhere: a supervisor outside the box holds the real credential and injects it at the network boundary. nono's phantom tokens, sbx header injection, microsandbox's guest-never-sees-it, Claude Code's masked env vars with injectHosts and SigV4 re-signing, Claude Code on the web's separate GitHub-token proxy, Codex's allowlist network_proxy, Cloudflare's outboundByHost handlers running in the Worker. A process sandbox with a good proxy beats a microVM with the key baked into the image. The fine print is what the proxy can see. Claude Code's and Codex's default proxies both decide on the CONNECT hostname and tunnel the TLS untouched, so an allowed host is an uninspected channel in both. Codex's limited mode and MITM hooks terminate TLS with a process-local CA to read the method and path; Claude Code's tlsTerminate does it only to rewrite credentials; Cloudflare's handlers see the whole request because the Worker is the client. None of that closes exfiltration: a secret in an allowed GET's query string passes every method clamp. The only host the agent cannot leak to is one you never allowed.",
  },
  {
