...
export const THREATS: Threat[] = [
  ...
  },
  {
    ...
    outcomes: {
      ...
      "claude-code": { verdict: "partial", note: "Credential file/env deny or mask, plus proxy substitution. Default disk reads are wide. Network is allowlist, not off, and an allowed host is an uninspected channel unless tlsTerminate is on." },
      codex: { verdict: "contained", note: "Default-deny network is the winning control; with network on, the allowlist proxy (deny wins, rebinding checks) keeps it, and limited mode can clamp an allowed host to reads inside HTTPS. Local still sees the workspace. Cloud strips environment secrets before the agent phase and sees only the clone." },
