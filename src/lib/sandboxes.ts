...
export const FAMILIES: ArchFamily[] = [
  ...
  },
  {
    ...
    layers: [
      ...
      { id: "vmm", label: "VMM — libkrun, Firecracker, Cloud Hypervisor, QEMU", kind: "boundary", blurb: "A small userspace monitor. Firecracker and Cloud Hypervisor are cloud-shaped. libkrun is embeddable and cross-platform. Docker sbx ships its own purpose-built VMM. hypeman can pick any of them — and which one decides whether a PCI device can pass through." },
...
export const SYSTEMS: System[] = [
  ...
  },
  {
    ...
    maker: "Docker",
    family: "microvm",
    familyNote: "Each agent session is a microVM with its own kernel and a private Docker daemon. The VMM is Docker's own, purpose-built, on the platform hypervisor — Hypervisor.framework, WHP, or KVM — not Firecracker and not libkrun, so it can be native on macOS and Windows.",
    oneLiner: "Treat the coding agent like a human developer: full docker build, no path back to the host.",
    role: "wrapper",
    vmm: "Purpose-built VMM on Hypervisor.framework / WHP / KVM",
    kernel: "dedicated",
    openSource: "Free standalone CLI, no Docker Desktop required; VMM not open. The same CLI can run the sandbox on Docker's cloud instead (--cloud).",
    platforms: "macOS, Windows, Linux (Ubuntu packages; KVM)",
    startup: "MicroVM boot; typically a short wait, then a full Linux userland",
    overhead: "Local: --cpus defaults to 0 (all host cores), memory to half of host RAM capped at 32 GiB. Cloud sandboxes (--cloud) default to 2 CPU / 4 GiB.",
    ...
    caveats: [
      ...
      "Defaults are generous (all cores, half of RAM) — set --cpus and -m per sandbox if two boxes must share the laptop",
      "GPU passthrough is experimental VFIO: x86_64 Linux, NVIDIA, an IOMMU, the card unbound from the host, and a driver bundle Docker rebuilds per release, behind platform.allowExperimentalFeatures",
  ...
  },
  {
    ...
    nestedDocker: "OCI-in-VM is the product. GPU via VFIO (device register --pci-address) or NVIDIA vGPU (--gpu-profile), on QEMU or Cloud Hypervisor. A full engine inside the guest is a guest concern.",
    ...
    caveats: [
      ...
      "The four hypervisors are not interchangeable: Firecracker and qemu-microvm cannot pass PCI devices, the macOS backend refuses VFIO outright, and vGPU wants QEMU — hypeman capabilities lists which runtime has gpu-passthrough",
  ...
  },
  {
    ...
    overhead: "A desktop per sandbox: an XFCE container at the light end, a 16 GB macOS guest at the heavy end. Fleet is billed capacity.",
    workspace: "The Image is the contract: apt_install, pip_install, run, copy and env are applied at launch (locally only — Fleet boots a prebuilt registry artifact). Nothing on the host is mounted; files go in by copy(). Ephemeral sandboxes are destroyed at the end; named ones persist and can be reconnected to.",
    network: "Open. QEMU user-mode NAT or the Docker bridge; expose() forwards ports at startup on bare-metal QEMU, Fleet turns them into authenticated named services. No allowlist and no credential proxy.",
    nestedDocker: "Linux VM: Docker Engine runs on the guest kernel. Container kind: no daemon inside. macOS guest: cannot host one. Windows: Docker Desktop needs nested virt from Hyper-V.",
    harness: "Not a Claude Code wrapper. Python and TypeScript SDKs plus MCP; the Cua agent loop drives computer-use models (Anthropic, OpenAI, local) against the sandbox. Install a coding CLI in the guest if you want one, then drive its terminal like any other window.",
    useCases: [
      "GUI agents that must also run code: click through an app, then run Python against the files it produced, in the same machine",
      "Evals and benchmarks (Cua-Bench) where every task starts from an immutable Image and the sandbox is thrown away",
      "The one SDK that hands an agent a Linux, macOS, Windows or Android desktop with the same API",
    ],
    notFor: [
      "Wrapping a coding CLI around your checkout — nothing is bind-mounted",
      "Anything that needs Sandbox.snapshot(): not implemented for local or Fleet in 0.4.3; prepare a boot artifact instead",
      "Egress policy or secret brokering; env() values are visible to anyone who can read the Image",
    ],
    security: "The wall is whatever the Image chose: a hypervisor for every VM kind, Docker namespaces on the host kernel for kind='container'. Actions inside a sandbox do not move the host mouse or touch host files — that is the product promise, and it holds because the GUI is the guest's. Secrets, egress and tokens are yours: no proxy, no broker, an open network.",
    caveats: [
      "kind='container' shares the host kernel (on a Mac, Docker Desktop's Linux VM) — the one path on this row without its own kernel",
      "Local builder layers are rejected by Fleet; the same Image behaves differently on the two paths",
      "Snapshots and forks are not implemented in cua-sandbox 0.4.3; tunnel.forward() is not implemented on local or pool transports",
    ...
    sources: [
      ...
      { label: "Choose and build a sandbox image", href: "https://cua.ai/docs/how-to-guides/sandbox/images" },
      { label: "github.com/trycua/cua", href: "https://github.com/trycua/cua" },
    ],
  },
  {
    id: "lume",
    name: "Lume",
    short: "Headless macOS VMs from an IPSW, shipped as OCI images",
    maker: "Cua (trycua)",
    family: "vm",
    familyNote: "The Virtualization.framework runtime underneath Cua's macOS sandboxes, usable on its own. lume create installs macOS from an Apple restore image, boots once, then stops the VM and mounts its Data volume on the host to patch the account, SSH, autologin and no-sleep settings offline — no Setup Assistant clicking. It boots again and verifies SSH. Same VZ as GhostVM and UTM; the difference is that the control channel is SSH, the image format is an OCI artifact, and everything is a CLI or a local REST API.",
    oneLiner: "lume create macos-tahoe --ipsw latest --unattended tahoe, then lume ssh — a vanilla Mac you can script, clone and push to a registry.",
    role: "runtime",
    vmm: "Virtualization.framework on Apple silicon. macOS guests from an IPSW; Linux guests too (--os linux).",
    kernel: "dedicated",
    openSource: "Open source, in trycua/cua (libs/lume). Telemetry on by default; lume config telemetry disable turns it off.",
    platforms: "Apple silicon, macOS 13+ host, 8 GB free RAM minimum (16 GB advised), 50 GB free disk per macOS guest. Guests: macOS Tahoe (verified) or Sequoia, or Linux.",
    startup: "A one-time IPSW install; then a full macOS boot per run. lume clone copies disk.img and nvram.bin as a pair to reproduce a prepared seed. No snapshots.",
    overhead: "Defaults: 4 CPUs, 8 GB, a 100 GB sparse disk (50 GB for Linux). Apple's Virtualization framework allows two macOS guests per host; one process may run a given VM at a time.",
    workspace: "The guest disk in Lume's storage locations. --shared-dir path:ro or path:rw mounts host directories over VirtioFS; --disk and --usb-storage attach images. lume push / lume pull move whole VMs through ghcr.io as OCI images.",
    network: "NAT. VNC is on in every display mode unless --vnc disabled. SSH is enabled by the unattended preset with the default lume / lume credentials. No allowlist, no proxy.",
    nestedDocker: "A macOS guest cannot host a VM, so no Docker inside it. A Linux guest (--os linux) runs Docker Engine on its own kernel with no nesting needed.",
    harness: "Not a wrapper. lume ssh <vm> <command> is the exec channel; lume serve exposes the same operations as a REST API on :7777, which is what the Cua Sandbox SDK talks to. Install Claude Code or Codex in the guest over SSH if that is the agent you want.",
    useCases: [
      "CI-shaped macOS sandboxes: create from an IPSW unattended, clone a golden seed per job, delete after",
      "Computer-use agents on a real macOS desktop, driven through cua-server in the guest and watched over VNC",
      "Shipping a prepared macOS VM to a team as an OCI image, and flipping SIP via Recovery when a workload needs it",
    ],
    notFor: [
      "A prompt-gated, clipboard-and-drag-and-drop workspace app — that is GhostVM",
      "More than two macOS guests on one Mac, or any nesting inside a macOS guest",
      "Snapshot-and-revert workflows: clone is the only checkpoint",
    ],
    security: "The same hardware wall as GhostVM with a thinner host-guest surface: SSH and VNC instead of vsock services, VirtioFS only when you ask. The sharp edges are defaults: the lume / lume account, SSH on, VNC on, NAT open, telemetry on. SIP state is a signed LocalPolicy, so it cannot be faked by editing the disk — lume sip goes through the VM's paired Recovery and verifies csrutil status after a real boot.",
    caveats: [
      "Change the lume / lume password before sensitive work; the docs say so",
      "Sequoia's unattended preset can still show the Setup Assistant accessibility step (issue #2155); Tahoe is the verified path",
      "The paravirtualized GPU reports an old Metal profile; Cua's capability shim unlocks newer kernels per process, experimentally",
    ...
    sources: [
      ...
      { label: "Lume CLI reference", href: "https://cua.ai/docs/reference/lume/cli-reference" },
      { label: "Lume limits and VM states", href: "https://cua.ai/docs/reference/lume/limits" },
      { label: "Metal capability shim (blog)", href: "https://cua.ai/blog/gpu-passthrough-macos-vms" },
    ],
  },
  {
    id: "claude-code",
    ...
    caveats: [
      ...
      "The egress proxy allowlists by host and is blind to TLS payloads by default — tlsTerminate in sandbox-runtime is experimental — so a permitted domain is a permitted channel, methods and paths included",
...
export const THREATS: Threat[] = [
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "contained", note: "Nothing on the host is mounted; files enter by copy(). The container kind has its own rootfs, every VM kind its own disk. rm inside hits the sandbox only." },
      lume: { verdict: "contained", note: "Guest disk. A --shared-dir path:rw is the exception; mount it :ro and it is not." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "partial", note: "Image.linux(kind='container') shares the host kernel — on a Mac, Docker Desktop's Linux VM. Every other Image boots its own: QEMU, Lume, Hyper-V. Pick the VM kind if this threat is in scope." },
      lume: { verdict: "contained", note: "A Virtualization.framework guest with its own XNU or Linux kernel. Escape needs a VZ or HVF bug." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "contained", note: "No socket is handed in. The container kind runs on the host's Docker, but the sandbox does not get its socket; a Linux VM carries its own daemon." },
      lume: { verdict: "contained", note: "None. A macOS guest cannot host a daemon; a Linux guest runs its own." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "partial", note: "Open network on every path, no proxy. env() values are in the Image spec for anyone who can read it, and the docs say not to put real secrets there. Host credentials stay out because nothing is mounted." },
      lume: { verdict: "partial", note: "Host files stay out unless shared. Inside: SSH on with lume / lume, VNC on, NAT open. Whatever you put in the guest can leave; nothing brokers it." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "contained", note: "There is no live checkout. The Image is immutable and the sandbox is a copy; destroy it and create another from the same Image." },
      lume: { verdict: "partial", note: "Project on the guest disk or in a clone: safe, delete and re-clone the seed. Project on a :rw shared dir: live, real wipe." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "partial", note: "Linux VM kind: Docker Engine runs on the guest kernel. Container kind: no daemon inside. macOS: cannot. Windows: Docker Desktop only with Hyper-V nested virt." },
      lume: { verdict: "partial", note: "macOS guest: no. Linux guest (--os linux): Docker Engine on its own kernel, no nesting needed — that is the path if a build must happen inside." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "partial", note: "Local: one host, your sandboxes. Fleet: Cua is the operator — pools, claims, authenticated named services — and that is the tenancy product. The container kind is not a tenant wall anywhere." },
      lume: { verdict: "partial", note: "Two macOS guests per host and one process per VM; lume serve is a local API. Strong wall, no control plane." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "contained", note: "Sandboxes are named; run as many as the host or the pool allows. macOS Images are capped at two per Mac by Apple." },
      lume: { verdict: "partial", note: "Two macOS guests, Apple's cap. Linux guests are not counted. Each VM is owned by one process." },
      "claude-code": {
        verdict: "n/a",
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "partial", note: "QEMU user-mode NAT and the Docker bridge both reach the host's LAN-facing services; 127.0.0.1 on the host is out of reach. Nothing filters it." },
      lume: { verdict: "partial", note: "NAT: the host's LAN address is one hop away, its loopback is not. Same as GhostVM and UTM." },
  ...
  },
  {
    ...
    outcomes: {
      ...
      "cua-sandbox": { verdict: "exposed", note: "Whatever the Image or your code hands the sandbox is the sandbox's. No broker, no per-tool policy; Fleet service URLs need the lease, that is all." },
      lume: { verdict: "exposed", note: "No proxy. The token you scp in over lume ssh is the guest's to use." },
...
export const HARNESSES = [
  {
    ...
    cells: {
      ...
      "cua-sandbox": "Install in the guest; the Cua agent loop drives a computer-use model instead",
      lume: "Install over lume ssh; no wrapper",
      "claude-code": "Native",
      codex: "—",
  ...
  },
  {
    ...
    cells: {
      ...
      utm: "Install in the guest",
      "cua-sandbox": "Install in the guest",
      lume: "Install over lume ssh",
      "claude-code": "—",
      codex: "Native",
      nono: "Wrap + signed profile",
      incus: "Install inside the machine",
  ...
  },
  {
    ...
    cells: {
      ...
      "cua-sandbox": "First-class: Python + TypeScript SDKs, MCP; shell, PTY, venv and GUI actions",
      lume: "CLI + REST API (lume serve); SSH exec",
      "claude-code": "No",
      codex: "No",
  ...
  },
  {
    ...
    cells: {
      ...
      "cua-sandbox": "The product: screenshots, a11y tree, clicks in any guest OS",
      lume: "VNC and a real macOS desktop; bring the driver",
...
export const MATRIX_ROWS: {
...
}[] = [
  {
    ...
    values: {
      ...
      "docker-sbx": "Purpose-built VMM microVM",
      microsandbox: "libkrun microVM",
      hypeman: "CH / Firecracker / QEMU / VZ — not interchangeable for devices",
      ...
      "cua-sandbox": "Docker container → QEMU / Hyper-V / Lume VM, per Image",
      lume: "Virtualization.framework macOS or Linux guest",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Shared (container kind); dedicated (every VM kind)",
      lume: "Dedicated guest (XNU or Linux)",
  ...
  },
  {
    ...
    label: "GPU",
    values: {
      yolobox: "Host device nodes: --gpus, --device; NVIDIA toolkit does the rest",
      "docker-sbx": "Experimental VFIO: x86_64 Linux + NVIDIA only, whole card, feature-flagged",
      microsandbox: "Not documented",
      hypeman: "VFIO passthrough or NVIDIA vGPU — QEMU or Cloud Hypervisor only; refused on macOS",
      cloudflare: "None",
      ghostvm: "Paravirtualized Metal (VZ), no passthrough",
      "agent-sandbox-vm": "Whatever the hypervisor gives a desktop guest; nothing scripted",
      utm: "Paravirtualized Metal for macOS guests; virgl for Linux; none for Windows",
      "cua-sandbox": "Per image: paravirtualized Metal + shim (Lume); none documented for the container or QEMU kinds",
      lume: "Paravirtualized Metal, old profile; Cua's capability shim, experimental",
      "claude-code": "A policy, not a device wall — the host's Metal / CUDA as before",
  ...
  },
  {
    ...
    values: {
      ...
      utm: "General VM app",
      "cua-sandbox": "Computer-use sandbox SDK",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Docker / QEMU / lume serve locally; Fleet pools in the cloud",
      lume: "lume serve on :7777, or the CLI",
      "claude-code": "None",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Your agent outside; cua-server inside answers shell, PTY, venv and GUI calls",
      lume: "Your agent outside; SSH into the guest",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Seconds (container) to an OS boot (VM); Fleet claims a booted machine",
      lume: "IPSW install once; macOS boot per run; clone as seed",
  ...
  },
  {
    ...
    values: {
      ...
      "docker-sbx": "All cores, half of RAM (≤32 GiB) by default; 2 CPU / 4 GiB in cloud",
      ...
      "cua-sandbox": "A desktop per sandbox: XFCE container to a 16 GB macOS guest",
      lume: "4 CPU / 8 GB / 100 GB sparse defaults; two macOS guests max",
  ...
  },
  {
    ...
    values: {
      ...
      utm: "Hypervisor",
      "cua-sandbox": "Hypervisor, or Docker namespaces for kind='container'",
      lume: "Hypervisor (VZ)",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Image is the contract: copy() in, fresh sandbox each time; no snapshots",
      lume: "Guest disk; --shared-dir :ro or :rw; clone the seed",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Open; QEMU NAT or Docker bridge; expose() ports",
      lume: "NAT; SSH + VNC on by default",
  ...
  },
  {
    ...
    values: {
      ...
      utm: "Install in the guest",
      "cua-sandbox": "Install in the guest; the GUI half is the point",
      lume: "Install over SSH",
  ...
  },
  {
    ...
    values: {
      ...
      "cua-sandbox": "Python + TypeScript SDKs, MCP",
      lume: "REST API + CLI",
...
export const MAC_HOSTS: MacHost[] = [
  ...
  {
    id: "macos-guest",
    name: "macOS guest · GhostVM / UTM / Lume / vmctl",
    ...
    layers: [
      ...
      { id: "hvf", label: "Virtualization.framework — two guests, siblings", kind: "boundary", blurb: "Apple's own VMM. GhostVM, UTM (Apple backend), Cua's Lume and glslang's vmctl all sit here; it is the only way to boot macOS as a guest, and it counts to two. Lume's docs state the cap plainly: the framework allows up to two macOS guests on a host." },
      ...
      { id: "tools", label: "Guest tools · vsock · SSH · VirtioFS", kind: "guest", blurb: "GhostTools or SPICE agents give clipboard, file transfer and port discovery; Lume uses plain SSH and VNC, enabled by patching the guest's Data volume offline before first login. Each is a hole through the wall; GhostVM prompts and quarantines, UTM and Lume mostly do not." },
