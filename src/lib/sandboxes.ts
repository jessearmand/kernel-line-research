...
export const SYSTEMS: System[] = [
  ...
  },
  {
    ...
    platforms: "Linux native. macOS via a Linux VM — Colima or Lima (MIT), or OrbStack (closed source, free for personal use, paid for commercial) — Darwin cannot host LXC.",
  ...
  },
  {
    ...
    caveats: [
      ...
      "The egress proxy decides on the CONNECT hostname and does not terminate TLS by default, so an allowed domain is an uninspected two-way channel — Anthropic's docs name github.com as an exfiltration path and domain fronting as the bypass. tlsTerminate (v2.1.199, experimental) exists for mask credential substitution and adds no content filtering; sandbox-runtime's filterRequest is where request-level policy lives. Codex's proxy has the same blind default but ships the missing gear: a limited mode and per-host MITM hooks",
  ...
  },
  {
    ...
    network: "Off by default in workspace-write. sandbox_workspace_write.network_access turns it on; features.network_proxy then makes it allowlist-first — deny wins, loopback and private ranges blocked unless allow_local_binding, DNS-rebinding checks, Unix sockets allowlisted. The proxy is HTTP CONNECT plus SOCKS5; in the default mode = \"full\" HTTPS is tunneled opaquely, exactly as blind as Claude Code's. mode = \"limited\" clamps to GET/HEAD/OPTIONS and switches on a process-local MITM CA so the clamp reaches inside HTTPS; [network.mitm.hooks] match host, methods and path prefixes after CONNECT is terminated (the README example strips Authorization on POST/PUT to api.github.com under /repos/openai/). Cloud: off in the agent phase; per-environment domain allowlist plus optional GET/HEAD/OPTIONS-only.",
    ...
    security: "The important split: local is a process sandbox on a shared kernel; cloud is OpenAI's container. Do not cite one when you mean the other. Default-deny network, and an allowlist proxy with rebinding checks once you open it, is the standout local control versus Claude Code — and where Claude Code's proxy stops at the hostname, Codex's limited mode and MITM hooks can read the method and path. The proxy filters commands only — web search, MCP, connectors, the browser and the model call itself have separate controls.",
    caveats: [
    ...
    sources: [
      ...
      { label: "codex-network-proxy README @ 36f0dbe (limited mode, MITM hooks)", href: "https://github.com/openai/codex/blob/36f0dbe796d9bb1a18a0fc0640ed08b3e1d54564/codex-rs/network-proxy/README.md" },
      { label: "network-proxy config.rs NetworkMode @ 36f0dbe", href: "https://github.com/openai/codex/blob/36f0dbe796d9bb1a18a0fc0640ed08b3e1d54564/codex-rs/network-proxy/src/config.rs#L363-L372" },
      { label: "network-proxy mitm.rs @ 36f0dbe (process-local CA)", href: "https://github.com/openai/codex/blob/36f0dbe796d9bb1a18a0fc0640ed08b3e1d54564/codex-rs/network-proxy/src/mitm.rs#L107-L111" },
    ],
  },
  {
    id: "nono",
...
export const THREATS: Threat[] = [
  ...
  },
  {
    ...
    outcomes: {
      ...
      "claude-code": { verdict: "partial", note: "Credential file/env deny or mask, plus proxy substitution. Default disk reads are wide. Network is allowlist, not off, and an allowed host is an exfiltration channel with or without tlsTerminate — termination exists for credential masking, not content filtering." },
...
export const MATRIX_ROWS: {
  ...
  },
  {
    ...
    values: {
      ...
      "claude-code": "Allowlist proxy on the CONNECT hostname, TLS-blind (Bash only)",
...
export const SYSTEM_CONTAINER_CASES: {
  ...
  },
  {
    ...
    verdict: "need",
    need: "Same shape as Villega — one Incus system container per project, root, systemd, native Docker — packaged as `coi` for five agents instead of one person's script, with two things on top: nftables egress modes (restricted, allowlist, open) and a monitor that pauses the box on bulk reads and kills it on a reverse shell. Villega calls it 'the closest thing to what I wanted' and built his own for OrbStack-and-remote-Linux parity, which coi has since added — on Colima or Lima as well as OrbStack. That is why this page's picker defaults to coi: Incus, coi and Colima are all open source, while OrbStack is closed source, free for personal use only and $8 per user per month for commercial work.",
    ...
    sources: [
      ...
      { label: "OrbStack pricing: free for personal use, paid for commercial", href: "https://orbstack.dev/pricing" },
      { label: "abiosoft/colima (MIT)", href: "https://github.com/abiosoft/colima" },
    ],
  },
  {
    id: "nested-docker",
